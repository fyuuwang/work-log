...
- 晨报由脚本机械生成：跑 `scripts/render_todo.py render --archive` → 读取 `morning.md` **原样推送**（AI 不重写格式，展示层零判断）。晨报含「待办（进行中 / 等待中，合并单一区，PM 视角两者难分；等待/阻塞对象用 @X 体现）/ 已完成（默认摘要，`--done-full` 看全量）/ ⏰待提醒（已逾期置顶）」三区块。
