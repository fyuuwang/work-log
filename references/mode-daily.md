...
- 从 .1 步获取当天所有 session_id，对每个 session_id 运行脚本提取 trace 摘要：
  ...
  ~/.workbuddy/binaries/python/versions/3.13.12/python.exe \
    ~/.workbuddy/skills/work-log/scripts/read_trace.py --session <session_id>
  ```
...
- 读取 `E:/OneDrive/Datas/03_中旅发展/AI_WorkPlace/work-log/state/last_run.json`（不存在则视为首次）。
...
- 处理完更新 `E:/OneDrive/Datas/03_中旅发展/AI_WorkPlace/work-log/state/last_run.json`：`last_processed_ms` = 现在，`processed_ids` = 旧值 ∪ 本次所有涉及 session_id（**累积并集**去重，绝不覆盖旧值；这样任意会话只会被处理一次）。
...
- **大总结**（给自己）：逐会话全量自阅，尽量保留原始密度与关键决策/产出。
  写入 `E:/OneDrive/Datas/03_中旅发展/AI_WorkPlace/work-log/daily/YYYY-MM-DD.md` 的"大总结"节。
...
`E:/OneDrive/Datas/03_中旅发展/AI_WorkPlace/work-log/usage/daily_usage.md`（APPEND ONLY，不覆盖）。

## 5. 落盘

- 工作日志：`E:/OneDrive/Datas/03_中旅发展/AI_WorkPlace/work-log/daily/YYYY-MM-DD.md`（不存在则新建，存在则追加当天小节；写前先 `mkdir -p` 确保 `daily/` 存在）。
- 用量：`E:/OneDrive/Datas/03_中旅发展/AI_WorkPlace/work-log/usage/daily_usage.md`（独立文件，用户已确认）。
